#tokenisation
~/mosesdecoder/scripts/tokenizer/tokenizer.perl -l en < ~/corpus/EnEsText/KDE4.es-en.en > ~/corpus/KDE4.es-en.tok.en -threads 12

~/mosesdecoder/scripts/tokenizer/tokenizer.perl -l es < ~/corpus/EnEsText/KDE4.es-en.es > ~/corpus/KDE4.es-en.tok.es -threads 12
...
mkdir ~/working
cd ~/working
nohup ~/mosesdecoder/scripts/training/train-model.perl -root-dir train -corpus ~/corpus/KDE4.es-en.clean -f en -e es --score-options '--GoodTuring'  -alignment grow-diag-final-and -reordering msd-bidirectional-fe -lm 0:3:$HOME/lm/KDE4.es-en.blm.es:8 -mgiza -external-bin-dir $HOME/mosesdecoder/tools  -cores 12 --parallel --parts 3  >& training.out &
