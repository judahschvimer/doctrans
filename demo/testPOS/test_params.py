...
def POS_tag(corpus_path, token_path):
    pcommand("{0}/scripts/training/wrappers/make-factor-pos.tree-tagger.perl -tree-tagger {1} -l en  {3}.clean.en  {3}.pos.en".format(moses_path, tagger_path, corpora_path, token_path), log_file)
    pcommand("{0}/scripts/training/combine_factors.pl {2}.clean.en {2}.pos.en > {2}.factored.en".format(moses_path, corpora_path, token_path), log_file)
    
    pcommand("{0}/scripts/training/wrappers/make-factor-pos.tree-tagger.perl -tree-tagger {1} -l {4}  {3}.clean.{4}  {3}.pos.{4}".format(moses_path, tagger_path, corpora_path, token_path, foreign), log_file)
    pcommand("{0}/scripts/training/combine_factors.pl {2}.clean.{3} {2}.pos.{3} > {2}.factored.{3}".format(moses_path, corpora_path, token_path, foreign), log_file)
    
    pcommand("{0}/scripts/training/clean-corpus-n.perl {1}.pos {2} en {1}.pos.clean 1 80".format(moses_path, token_path, foreign), log_file)
    pcommand("{0}/scripts/training/clean-corpus-n.perl {1}.factored {2} en {1}.factored.clean 1 80".format(moses_path, token_path, foreign), log_file)
